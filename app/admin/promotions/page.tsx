...
export default function AdminPromotions() {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (bannerFile) {
        ...
        uploadForm.append('purpose', 'promotions')
        const uploadResponse = await fetch('/api/uploads', { method: 'POST', body: uploadForm })
        const uploadPayload = await uploadResponse.json()
        if (!uploadResponse.ok) throw new Error(uploadPayload.error?.message || 'Gagal mengunggah banner ke Cloudflare R2')
  ...
  return <AdminLayout>
    <div className="space-y-6">
      ...
      <AdminNotice message={error} tone="danger" />
