      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {courses.length ? <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {courses.map((course, index) => <motion.article key={course.id} initial={{ opacity: 0, y: 28 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5, delay: index * 0.08 }} className="group overflow-hidden rounded-2xl border border-border bg-background transition-transform duration-500 hover:-translate-y-2">
            ...
            <div className="space-y-5 p-6"><h3 className="font-display text-2xl leading-tight">{course.title}</h3><p className="text-sm leading-relaxed text-text-muted">{course.description}</p><div className="grid grid-cols-2 gap-3 border-y border-border py-4 text-xs text-text-light"><span className="flex items-center gap-2"><Clock3 className="h-4 w-4 text-primary" />{course.duration}</span><span className="flex items-center gap-2"><CalendarDays className="h-4 w-4 text-primary" />{course.schedule}</span><span className="col-span-2 flex items-center gap-2"><UserRound className="h-4 w-4 text-primary" />{course.instructor}</span></div><a href={course.enrollmentUrl || `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(`Halo, saya ingin mendaftar kursus ${course.title} di Beauty Salon Elin.`)}`} target={course.enrollmentUrl ? undefined : '_blank'} rel={course.enrollmentUrl ? undefined : 'noopener noreferrer'} className="inline-flex items-center gap-2 font-semibold text-primary transition-colors hover:text-primary-light">Tanya tentang kelas <ArrowUpRight className="h-4 w-4 transition-transform group-hover:translate-x-1 group-hover:-translate-y-1" /></a></div>
          </motion.article>)}
